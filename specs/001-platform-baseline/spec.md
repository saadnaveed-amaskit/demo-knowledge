...
**Created**: 2026-07-07
**Status**: Approved
**Input**: "Retail Nucleus pricing platform baseline covering all screens, personas, EARS requirements, and Gherkin acceptance scenarios"
...
> **Speckit note:** This platform baseline was created with native Speckit infrastructure (`.specify/scripts/powershell/create-new-feature.ps1`), which created and checked out git branch `001-platform-baseline` in the `knowledge/` repo and scaffolded `specs/001-platform-baseline/spec.md` from `spec-template.md`. This command explicitly authorizes that branch/folder creation. The scaffolded template was then populated to satisfy this command's required platform spec structure; the canonical baseline location is recorded in `knowledge/specs/platform-baseline-context.md`.
>
> **History note:** an earlier fixed-path spec/plan existed at `knowledge/specs/000-platform/` (approved, then removed by the human in commit `cd2395e "Remove old files before Speckit-native"`). `001-platform-baseline` is the new canonical platform baseline.
...
- `knowledge/.specify/templates/spec-template.md` — Speckit spec conventions (scaffold source).
- `knowledge/specs/**` — empty at time of authoring (prior `000-platform` removed).
...
|---|---|---|---|---|---|---|
| SLICE-00 | Repo scaffolding & quality gates | Pinned stacks, BDD/unit harnesses, `npm run check` | frontend, backend | — | (foundation) | `npm run check` + smoke BDD run |
| SLICE-01 | Platform shell & navigation | Sidebar sections, routing, global brand/channel filters | frontend | SLICE-00 | REQ-SHELL-001, REQ-SHELL-002 | Nav renders; routes resolve; filters present |
...
_This spec describes WHAT and WHY, not implementation HOW. It contains no code, tasks, or final API/implementation design. It is `Approved` at 07/07/2026. `/create-platform-plan` may run._
