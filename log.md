...
## 2026-07-10 — SLICE-12: Measurement (SHIPPED)

- **Baseline:** `001-platform-baseline`
- **Slice status:** Complete
- **frontend:** PR [#14](https://github.com/saadnaveed-amaskit/demo-frontend/pull/14) merged to `main` — merge SHA `6bf530564f604a8c9d81d93f076dfe41cb0dfa43`. `MeasurementScreen` at `/measurement`: experiment list → detail view with a per-block balance chart (`@nivo/bar`), per-cluster arm-move controls, cost-of-control acknowledgment (shadcn `Switch`), a Go-Live button disabled with a blocked-reason tooltip until eligible, and a live-readout section (KPI tiles, per-cluster contribution, verdict banner, scale/kill actions). No new pinned-stack dependency introduced.
- **backend:** PR [#13](https://github.com/saadnaveed-amaskit/demo-backend/pull/13) merged to `main` — merge SHA `5f46cc599365cfcd2b3180776eacaa93f5086fd2`. `MeasurementController`/`MeasurementService` (6 endpoints: list/get experiments, move cluster arm, acknowledge cost, go-live, scale, kill). Block balance derived on every read from per-metric match ratios (min/max >= 0.8 rule, REQ-MEAS-014) plus arm presence (`"Missing-an-arm"` if a block loses its only treatment or control cluster). Go-Live gated on all blocks Balanced + cost acknowledged (REQ-MEAS-005/006); moving a cluster to `"control"` nulls its displayed `mlPrice` while the underlying ML recommendation is retained internally and restored if moved back (REQ-MEAS-007). `backend/contracts/api-contract.md` updated with the 6 new endpoints and 8 new schemas, hand-authored directly since `api-contract.yaml` no longer exists on `main`. This slice's preparation gate also introduced the backend's first-ever Cucumber/BDD tooling (`@cucumber/cucumber`, `ts-node`, in-process test app via `@nestjs/testing`) for API-level acceptance scenarios exercising the API directly, per a mid-preparation correction requiring backend API Gherkin coverage (a Jest contract test alone was judged insufficient for externally observable API behavior).
- **Requirements:** REQ-MEAS-001…007, 009…011, 014 covered. REQ-MEAS-002's cost/time-to-signal figures surfaced only as a boolean acknowledgment, not separate computed fields. **REQ-MEAS-008 (streaming trend + play/pause/restart), REQ-MEAS-012 (closed-loop feedback into Autonomy's accuracy record), and REQ-MEAS-013 (reopen setup preserving overrides) deferred** — none were covered by the approved preparation-gate test scaffolding (Jest mock and Cucumber scenarios covered only list/get/move/acknowledge-cost/go-live/scale/kill), so implementing them would have exceeded reviewed scope. REQ-MEAS-012 is also explicitly flagged `[NEEDS CLARIFICATION]` in the approved spec (Open Question 9).
- **Validation:** `specs/001-platform-baseline/validation/SLICE-12.md` — Shipped (backend 133/133 tests + 4/4 backend API Cucumber scenarios passed against the real implementation, `npm run check` green on both repos; frontend 35/35 BDD scenarios — 3 new + no regressions — `npx vitest run` and `npm run check` green). 0 formal patch attempts. One test-infrastructure observation (not a code defect): rerunning the frontend BDD suite twice against the same long-running backend dev server leaked mutated in-memory state between runs — resolved by restarting the dev server, same class of issue as SLICE-11's kill-switch state leak.
- **Contracts promoted:** `contracts/slice-12-measurement/contract.md` created directly as **Stable** during knowledge finalization (no draft existed from the preparation gate — contract authoring was explicitly deferred to implementation per `tasks.md`, and was not created during implementation either; filled in now to keep the per-slice contract convention consistent).
- **Open items carried forward:** ORM/DB `[NEEDS CLARIFICATION]`; in-memory store resets on restart; REQ-MEAS-008/012/013 deferred (see Requirements above); verdict/win-kill boundary day-over-day progression is a v1 deterministic placeholder (only the pre-seeded "already past the win boundary" experiment exercises the win-verdict path); `backend/contracts/api-contract.yaml` remains absent from `main`.
- **Next:** SLICE-13 (Like-Item Mapping / Cold Start, depends on SLICE-02 ✓) and SLICE-14 (Configuration, depends on SLICE-01 ✓) both remain `Approved` and are the next candidates by dependency order.
